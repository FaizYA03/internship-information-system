@extends('lab.layouts.unified', ['title' => 'Edit Laboratorium'])
...
<style>
    ...
    .kode-input-group input:read-only {
        background: #f0f4ff;
        border-color: #c7d5f8;
        cursor: default;
    }
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h4 class="fw-bold mb-1 text-dark">
                    <i class="bi bi-pencil-square me-2 text-primary"></i>Edit Laboratorium
                </h4>
                <p class="text-muted small mb-0">
                    Memperbarui informasi <strong>{{ $labor->nama_labor }}</strong>
                    @if($labor->kode)
                        &nbsp;•&nbsp; <span class="kode-badge"><i class="bi bi-tag-fill"></i>{{ $labor->kode }}</span>
                    @endif
                </p>
            </div>
            <a href="{{ route('lab.admin_new.laboratorium.show', $labor->id) }}" class="btn btn-outline-secondary rounded-pill px-4">
    ...
    <div class="col-lg-9">
        <div class="card border-0 shadow-sm" style="border-radius: 16px;">
            <div class="card-body p-4">
                ...
                <form action="{{ route('lab.admin_new.laboratorium.update', $labor->id) }}"
                      method="POST"
                      enctype="multipart/form-data"
                      id="editLaboratoryForm">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        <div class="form-section-title">
                            <i class="bi bi-grid-3x3-gap me-2"></i>Jenis Laboratorium
                        </div>

                        {{-- Hidden select tetap dikirim ke server --}}
                        <select name="jenis_labor" id="jenis_labor" class="d-none" required>
                            @foreach($jenisOptions as $jenis)
                                <option value="{{ $jenis->nama }}"
                                    {{ old('jenis_labor', $labor->jenis_labor) === $jenis->nama ? 'selected' : '' }}>
                                    {{ $jenis->nama }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        @php
                        $selectedJenis = old('jenis_labor', $labor->jenis_labor ?? 'Lainnya');
                        $warnaHex = [
                        ...
                        <div class="row g-2" id="jenisCards">
                            @foreach($jenisOptions as $jenis)
                                @php
                                    ...
                                    // Prefix is now solely generated based on name akronim
                                @endphp
                    ...
                    <div class="mb-4">
                        ...
                        <div class="form-text mt-2">
                            ...
                            <span id="kodePreview" class="ms-auto" style="display: none;">
                                <span class="kode-badge"><i class="bi bi-tag-fill"></i><span id="kodePreviewText"></span></span>
                            </span>
                        </div>
                        @error('kode')
                    ...
                    <div class="mb-4">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label for="status_penggunaan" class="form-label fw-semibold small">Status Penggunaan Saat Ini</label>
                                <select name="status_penggunaan" id="status_penggunaan" class="form-select @error('status_penggunaan') is-invalid @enderror">
                                    <option value="kosong" {{ old('status_penggunaan', $labor->status_penggunaan) == 'kosong' ? 'selected' : '' }}>Tersedia / Kosong</option>
                                    <option value="digunakan" {{ old('status_penggunaan', $labor->status_penggunaan) == 'digunakan' ? 'selected' : '' }}>Sedang Digunakan</option>
                                </select>
                                <div class="form-text small mt-2">
                                    <i class="bi bi-info-circle me-1"></i> Status ini akan otomatis berubah menjadi "Digunakan" jika ada jadwal aktif.
                                </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number"
                                           ...
                                           value="{{ old('kapasitas', $labor->kapasitas ?? 30) }}"
                                           min="1" max="500"
                                           placeholder="30">
                                    @error('kapasitas')
                                        <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="col-md-6">
                                ...
                                @if($labor->penanggungJawabUser)
                                    <small class="text-muted mt-1 d-block">
                                        ...
                                        Saat ini: <strong>{{ $labor->penanggungJawabUser->nama }}</strong>
                                    </small>
                                @endif
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text"
                                           ...
                                           value="{{ old('teknisi', $labor->teknisi) }}"
                                ...
                                @if($labor->teknisiUser)
                                    <small class="text-muted mt-1 d-block">
                                        <i class="bi bi-check-circle-fill text-success me-1"></i>
                                        Saat ini: <strong>{{ $labor->teknisiUser->nama }}</strong>
                                    </small>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <textarea id="deskripsi"
                                          ...
                                          rows="3"
                                          placeholder="Deskripsi singkat tentang laboratorium ini...">{{ old('deskripsi', $labor->deskripsi) }}</textarea>
                                @error('deskripsi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <textarea id="fasilitas"
                                          ...
                                          rows="3"
                                          placeholder="Contoh: 30 unit komputer, AC, Proyektor, Jaringan LAN...">{{ old('fasilitas', $labor->fasilitas) }}</textarea>
                                @error('fasilitas')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-4">
                        ...
                        @if($labor->foto)
                            <div class="mb-3">
                                <label class="form-label fw-semibold small">Foto Saat Ini</label>
                                <div>
                                    <img src="{{ Storage::url($labor->foto) }}"
                                         alt="Foto {{ $labor->nama_labor }}"
                                         class="img-thumbnail"
                        ...
                        <label for="foto" class="form-label fw-semibold small">
                            {{ $labor->foto ? 'Ganti Foto (Opsional)' : 'Upload Foto (Opsional)' }}
                        </label>
                        <input type="file"
                               id="foto"
                               name="foto"
                        ...
                        <div id="fotoPreviewContainer" class="mt-3 d-none">
                            <label class="form-label fw-semibold small text-muted">Preview foto baru:</label>
                            <div>
                    ...
                    <div class="d-flex justify-content-end gap-2 pt-3 border-top">
                        <a href="{{ route('lab.admin_new.laboratorium.show', $labor->id) }}"
...
<script>
    ...
    // ======================================================
    // Bangun jenisConfig dinamis dari DB
    const jenisConfig = {
        @foreach($jenisOptions as $jenis)
    ...
    let selectedJenis = '{{ old('jenis_labor', $labor->jenis_labor ?? 'Lainnya') }}';
    ...
    function selectJenis(jenis) {
        ...
        // Update kode preview jika sudah ada generate
        const kodeInput = document.getElementById('kode');
        if (!kodeInput.value || kodeInput.value.startsWith('LAB-')) {
            // Auto-update saran kode
            updateKodePreview(prefix);
        }
    }

    function updateKodePreview(prefix) {
        const previewEl = document.getElementById('kodePreview');
        const previewText = document.getElementById('kodePreviewText');
        previewEl.style.display = 'inline-flex';
        previewText.textContent = prefix + '-XXX (akan di-generate saat simpan)';
    }
    ...
    function generateKode() {
        ...
        .then(data => {
            if (data.kode) {
                ...
                const previewText = document.getElementById('kodePreviewText');
                previewEl.style.display = 'inline-flex';
                previewText.textContent = data.kode;
                ...
                kodeInput.style.transition = 'background 0.5s';
                setTimeout(() => {
                    kodeInput.style.background = '#f0f4ff';
                }, 600);
            }
        })
        .catch(() => {
    ...
    // ======================================================
    // Live preview: saat user ubah nama, tampilkan akronim di tombol
    // ======================================================
    ...
    // ======================================================
    document.getElementById('editLaboratoryForm').addEventListener('submit', function () {
