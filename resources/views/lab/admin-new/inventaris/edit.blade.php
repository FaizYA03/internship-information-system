@extends('lab.layouts.unified', ['title' => 'Edit Inventaris'])
...
<div class="d-flex align-items-center gap-3 mb-4">
    <a href="{{ route('lab.admin_new.inventaris.show', $inventaris->id) }}"
    ...
    <div>
        <h4 class="fw-bold mb-0 text-dark">Edit Inventaris</h4>
        <p class="text-muted small mb-0">
            <i class="bi bi-tag me-1"></i>{{ $inventaris->kode_inventaris ?? 'Tanpa Kode' }}
            &nbsp;•&nbsp;
...
<form action="{{ route('lab.admin_new.inventaris.update', $inventaris->id) }}"
    ...
    <div class="row g-4">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="kode_inventaris"
                               ...
                               placeholder="Contoh: INV20240001"
                               maxlength="50">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="jenis" class="form-select @error('jenis') is-invalid @enderror" required>
                            <option value="Alat" {{ old('jenis', $inventaris->jenis) == 'Alat' ? 'selected' : '' }}>Alat</option>
                            <option value="Bahan" {{ old('jenis', $inventaris->jenis) == 'Bahan' ? 'selected' : '' }}>Bahan</option>
                        </select>
                        @error('jenis')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="kategori" class="form-select @error('kategori') is-invalid @enderror" required>
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->nama }}" {{ old('kategori', $inventaris->kategori) == $cat->nama ? 'selected' : '' }}>
                                    {{ $cat->nama }}
                                </option>
                            @endforeach
                            @if($categories->isEmpty())
                                <option value="Umum" {{ old('kategori', $inventaris->kategori) == 'Umum' ? 'selected' : '' }}>Umum (Default)</option>
                            @endif
                        </select>
                        @error('kategori')
                            <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="form-section">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="labor_id" class="form-select @error('labor_id') is-invalid @enderror">
                            <option value="">-- Pilih Laboratorium --</option>
                            @foreach($laboratories as $lab)
                                <option value="{{ $lab->id }}"
                                    {{ old('labor_id', $inventaris->labor_id) == $lab->id ? 'selected' : '' }}>
                                    {{ $lab->nama_labor }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="lokasi"
                               ...
                               placeholder="Contoh: Rak A Lantai 2, Lemari B...">
                        @error('lokasi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="kondisi" class="form-select @error('kondisi') is-invalid @enderror" required>
                            ...
                            @foreach(['Sangat Baik','Baik','Rusak Ringan','Rusak Sedang','Rusak Berat'] as $k)
                                <option value="{{ $k }}"
                                    {{ old('kondisi', $inventaris->kondisi) == $k ? 'selected' : '' }}>
                                    {{ $k }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <textarea name="deskripsi" rows="3"
                                  ...
                                  placeholder="Tuliskan deskripsi singkat tentang alat ini...">{{ old('deskripsi', $inventaris->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-12">
                        ...
                        <textarea name="spesifikasi" rows="3"
                                  ...
                                  placeholder="Contoh: Voltase: 220V, Daya: 500W, Frekuensi: 50Hz...">{{ old('spesifikasi', $inventaris->spesifikasi) }}</textarea>
                        @error('spesifikasi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
        ...
        <div class="col-lg-4">
            ...
            <div class="form-section">
                ...
                <div id="previewContainer" onclick="document.getElementById('gambarInput').click()" style="cursor:pointer;">
                    @if($inventaris->gambar)
                        <img src="{{ asset('storage/' . $inventaris->gambar) }}"
                             class="preview-image" id="previewImg" alt="Preview">
                        <p class="text-muted small text-center mt-2">
                            <i class="bi bi-pencil me-1"></i>Klik untuk ganti foto
                        </p>
                    @else
                        <div class="preview-placeholder" id="previewPlaceholder">
                            <i class="bi bi-cloud-upload fs-2 mb-2"></i>
                            <span class="small">Klik untuk upload foto</span>
                            <span class="small text-muted">JPG, PNG (max 2MB)</span>
                        </div>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-12">
                        ...
                        <select name="sumber_dana" class="form-select @error('sumber_dana') is-invalid @enderror">
                            ...
                            @foreach(['APBN','BOS','Hibah','Lainnya'] as $s)
                                <option value="{{ $s }}"
                                    {{ old('sumber_dana', $inventaris->sumber_dana) == $s ? 'selected' : '' }}>
                                    {{ $s }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <input type="number" name="stok_minimum" min="0"
                               ...
                               value="{{ old('stok_minimum', $inventaris->stok_minimum ?? 0) }}"
                               placeholder="0">
                        @error('stok_minimum')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="d-flex flex-column gap-2">
                ...
                <button type="button" class="btn btn-outline-danger w-100 rounded-3"
                    ...
                    <i class="bi bi-trash me-2"></i>Hapus Inventaris
                </button>
            </div>
...
{{-- Delete Form HARUS di LUAR form edit agar tidak nested --}}
<form id="deleteForm" action="{{ route('lab.admin_new.inventaris.destroy', $inventaris->id) }}"
...
<script>
    ...
    // Character counter untuk nama
    const namaInput = document.getElementById('namaInventaris');
    const namaCount = document.getElementById('namaCount');
    function updateCount() {
        namaCount.textContent = namaInput.value.length + '/255';
    }
    namaInput.addEventListener('input', updateCount);
    updateCount(); // initial call

    // Konfirmasi hapus
    function confirmDelete(id) {
        if (confirm('Yakin ingin menghapus inventaris ini? Tindakan tidak dapat dibatalkan.')) {
    ...
    document.getElementById('editForm').addEventListener('submit', function(e) {
        ...
        if (!valid) {
            ...
            alertEl.className = 'alert alert-danger alert-dismissible fade show rounded-3 mb-4';
            alertEl.innerHTML = '<i class="bi bi-exclamation-triangle me-2"></i>Harap isi semua field yang wajib diisi! <button type="button" class="btn-close" data-bs-dismiss="alert"></button>';
            document.querySelector('.col-lg-8').prepend(alertEl);
        }
    });
</script>
@endsection
